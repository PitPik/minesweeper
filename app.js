;(function(undefined) {
  ...
  function createBoard(rowcol, mineCount, cbRow, cbCol, cbRowEnd) {
    for (var board =[], n = rowcol[0]; n--; ) {
      ...
      for (var m = rowcol[1]; m--; ) {
        board[n][m] = { mark: '', done: false, mine: false, x: n, y : m };
  ...
  function checkAround(board, item, cb, doCheck, foundMarked) {
    lookAround(board, item.x, item.y, function(row, col, foundItem) {
      if (foundMarked !== undefined) {
        if (foundItem.mark === marked) foundMarked++;
      } else if (doCheck || !foundItem.mine && !foundItem.done) {
  ...
  function checkAll(items) {
    for (var x = items.length; x--; ) if (!items[x].mine || items[x].mark !== marked) return;
    return true;
  }
  ...
  function checkItem(board, item, cb, doMark) {
    if (item.done && doMark !== undefined) {
      ...
      checkAround(board, item, cb, false, 0) === item.arround && checkAround(board, item, cb, true);
  ...
  function reset() {
    ...
    return { won: undefined, row: +values[0],  col: +values[1], mines: +values[2] };
  }
  ...
  function createView() {
    ...
    views.gameBoard.innerHTML = html;
    ...
    views.check = function(e) { // fresh event listener
      ...
      if (!views.interval) views.interval = setInterval(function() {
        views.timer[textContent]++;
      }, 1000);
      state.won = checkItem(board, board[row][col], function(item) {
        updateItemView(views.gameBoard, item);
      }, e.type === 'contextmenu');
      updatUIView(board, state);
  ...
  function updatUIView(board, state) {
    ...
    views.counter[textContent] = state.mines - getItems(board, 'mark', 'marked', []).length;
  }

  function updateItemView(gameBoard, item) {
    var cell = gameBoard.children[item.x].children[item.y];

    cell[className] = item.done ? ' color-' + item.arround : ' hidden ' + item.mark;
